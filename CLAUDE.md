...
3. JS/CSS/HTML geändert? → **Cache-Buster** hochzählen: das `?v=…` in `admin.html` (admin.js+admin.css),
   `overlay.html`, `trade.html`, `collection.html`. OBS/WebView cachen sonst alte Dateien.
...
- **OBS cacht Overlays hart**: nach JS-Änderung Cache-Buster hochzählen; der User muss die Browserquelle
  ggf. per „Cache aktualisieren" neu laden. Server sendet zwar `no-store`, aber sicher ist sicher.
